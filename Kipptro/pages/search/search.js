...
(function () {
	...
	ui.Pages.define(searchPageURI, {
		...
		_generateFilters: function () {
			...
			this._filters.push({ results: null, text: "Favorites", predicate: function (item) { return item.is_favorite; } });
		},

		// This function executes each step required to perform a search.
		_handleQuery: function (element, args) {
			var originalResults;
			this._lastSearch = args.detail.queryText;
			...
			this._searchData(args.detail.queryText);
		...
		_itemInvoked: function (args) {
			args.detail.itemPromise.done(function itemInvoked(item) {
				return WinJS.Navigation.navigate('/pages/clip/clip.html', { item: item });
			});
		},

		// This function colors the search term. Referenced in /pages/search.html
		// as part of the ListView item templates.
		_markText: function (text) {
			if (!text) text = "";
			return text ? text.replace(this._lastSearch, "<mark>" + this._lastSearch + "</mark>") : text;
		},
		...
		_searchData: function (queryText) {
			...
			if (window.Kippt) {
				Kippt.searchClips(queryText).then(function(results) {
					var originalResults;
					var json = JSON.parse(results.responseText);
					if (!json.objects) {
						originalResults = new WinJS.Binding.List();
					}
					else {
						originalResults = new WinJS.Binding.List(json.objects);
					}
					...
					self._populateFilterBar(elem, originalResults);
